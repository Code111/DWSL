...
from .BHCGSA import BHCGSA

class backbone(nn.Module):
    def __init__(self, c_in: int, pad_size:int, local_size:int, context_window: int, target_window: int, patch_len: int, stride: int,
        ...
        # Backbone
        self.backbone = TSTiEncoder(c_in, pad_size=pad_size, local_size=local_size, patch_num=patch_num, patch_len=patch_len,
...
class TSTiEncoder(nn.Module):
    def __init__(self, c_in, patch_num, patch_len, pad_size, local_size,n_layers=3,d_model=128,  dropout=0.,
        ...
        # Encoder
        self.encoder = TSTEncoder(c_in,q_len, d_model, pad_size, local_size=local_size, dropout=dropout, n_layers=n_layers)
...
class BHCGSAM(nn.Module):
    def __init__(self, c_in, q_len, dim, pad_size, local_size,mlp_ratio=4, drop_path=0., act_layer=nn.ReLU, norm_layer=nn.LayerNorm):
        super().__init__()
        ...
        self.mlp = Mlp(in_features=q_len, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop_path)
        self.BHCGSA = BHCGSA(dim=dim, local_size=local_size, attn_drop=drop_path, num_heads=2, proj_drop=drop_path)

    def forward(self, x):
        x = rearrange(x, '(b n) out_seg_num d_model -> (b out_seg_num) n d_model', n = self.c_in,out_seg_num=self.seg_num)
        dim_size = x.size(1)
        x = torch.nn.functional.pad(x, (0, 0, 0, self.pad_size))
        x = self.norm1(x + self.drop_path(self.BHCGSA(x)))
        x = x[:, :dim_size, :]
...
class TSTEncoder(nn.Module):
    def __init__(self, c_in, q_len, d_model, pad_size, local_size, dropout=0., n_layers=1):
        super().__init__()
        ...
        for i in range(n_layers):
            self.layers1.append((BHCGSAM(c_in, q_len, d_model, pad_size, local_size, mlp_ratio=4, drop_path=dropout)))
